...
nflux_struct* nf)
{
	...
	double drate;
	double Croot_frac=0.05,Leaf_frac=0.05;
	...
	else
	{
		...
		//	printf("凋落时间=%lf\n",ndays);
			if (ndays == 1.0)
			{
				...
				//最后一天收获
			/*	if(epc->epctype == 141|| epc->epctype == 142 || epc->epctype == 143 )//假如是农田，则将叶子收割，不作为凋落物
				{
					cs->livecrootc=0;
			...
			//added 2014-12-10   农田生态系统，地上生物量被收割，认为5%凋落。
				if(epc->epctype == 141|| epc->epctype == 142 || epc->epctype == 143)
				{
					leaflitfallc = cs->leafc * 0.5;
					cs->leafc = 0.0;
					cs->leafc_storage = 0;
					//cs->frootc_storage = 0;
					cs->deadcrootc = 0;
