...
int decomp_soil(soilvar_struct soil_sum[], double tsoil_bgc,const epconst_struct* epc, epvar_struct* epv, 
...
nstate_struct* ns, nflux_struct* nf, ntemp_struct* nt)
{
	...
	int nlimit;
	double ratio;
	double tsoil,tsoil_sum,epv_psi,epv_psi_sum;
	FILE *fp_error;
	tsoil_sum=0;
	epv_psi_sum=0;
	//定义个非冻土碳含量的系数。
	...
	epv_psi = epv_psi_sum/6/102;   //unit transter m->MPa
	if(epv_psi > 0)
		epv_psi = 0;
	*/
	tsoil = soil_sum[0].FSM_soilT;
	epv_psi = soil_sum[0].FSM_psi;
	if(tsoil >= 0 || tsoil< -0)
	{
		;//printf("tsoil=%lf, 土壤水势=%lf,土壤水势_bgc=%lf,soilT_error=%d\n",tsoil,epv_psi,epv->psi,cs->soilT_error_mark);
	}
	else   //捕获tsoil为错误值的情况；
	{
		tsoil = tsoil_bgc;
	...
	*/
	...
	{
		//w_scalar = log(minpsi/epv_psi)/log(minpsi/maxpsi);
		w_scalar = log(minpsi/epv_psi)/log(minpsi/maxpsi);
	}
