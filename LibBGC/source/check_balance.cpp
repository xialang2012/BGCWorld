...
{
	int ok=1;
	static double old_balance,old_store,old_out,old_in;
	double in, out, store, balance;
	
	/* DAILY CHECK ON WATER BALANCE */
	
	/* sum of sources */
	in = ws->prcp_src;// + ws->soilw_add;
	//if(ws->soilw_add)
	//			printf("soilw_add=%lf",ws->soilw_add);
	/* sum of sinks */
	...
	if (!first_balance)
	{
		if (fabs(old_balance - balance) > 1e8)// 1e-4)
		{
			bgc_printf(BV_ERROR, "FATAL ERRROR: Water balance error:\n");
			bgc_printf(BV_ERROR, "in = %lf ;store = %lf; out= %lf; old_in = %lf ;old_store = %lf; old_out= %lf;\n",in,store,out,old_in,old_store,old_out);//
			bgc_printf(BV_ERROR, "outflow_snk = %lf ;soilevap_snk = %lf; snowsubl_snk= %lf;canopyevap_snk = %lf ;trans_snk = %lf;\n",ws->outflow_snk,ws->soilevap_snk,ws->snowsubl_snk,ws->canopyevap_snk,ws->trans_snk);//
...
int check_carbon_balance(cstate_struct* cs, int first_balance)
{
	...
	if (!first_balance)
	{
		if (fabs(old_balance - balance) > 1e8)//1e-8)
		{
