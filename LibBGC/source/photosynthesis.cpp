...
int total_photosynthesisTimeRes(std::vector<StationDataFlux*> &sfData, const cstate_struct* cs, const double albedo, const metvar_struct* metv, const epconst_struct* epc,
	epvar_struct* epv, cflux_struct* cf, psn_struct *psn_sun, psn_struct *psn_shade, const int yearS, const int daysS)//,FILE *fp_ci, int mode)
{
  ...
  /* set the input variables */
	...
	psn_sun->g = epv->gl_t_wv_sun * 1e6 / (1.6*R*(metv->tday + 273.15));
	psn_sun->dlmr = epv->dlmr_area_sun;
	if (ok && photosynthesisTimeRes(sfData, epc, epv, cs, albedo, psn_sun, metv, yearS, daysS, 1))//,1,fp_ci,mode))
	{
		bgc_printf(BV_ERROR, "Error in photosynthesis() from bgc()\n");
		ok = 0;
	}
	...
	psn_shade->g = epv->gl_t_wv_shade * 1e6 / (1.6*R*(metv->tday + 273.15));
	psn_shade->dlmr = epv->dlmr_area_shade;
	if (ok && photosynthesisTimeRes(sfData, epc, epv, cs, albedo, psn_shade, metv, yearS, daysS, 0))//,0,fp_ci,mode))
	{
		bgc_printf(BV_ERROR, "Error in photosynthesis() from bgc()\n");
		ok = 0;
	}
...
int photosynthesisTimeRes(std::vector<StationDataFlux*> &sfData, const epconst_struct* epc, epvar_struct* epv,
	const cstate_struct* cs, const double albedo, psn_struct *psn, const metvar_struct* metv, const int yearS, const int daysS, const int sunorshade)//,int sun,FILE *fp_ci, int mode)
{
	int ok = 1;
	...
	double totalA = 0;
	int totalNum = 0;
	for (int i = 1; i <= 24 * 60/ timeRes; ++i)
	{
		psn_struct psnT = *psn;
		metvar_struct metvT = *metv;
		metvT.swavgfd = sfData[dayCurr]->Rd;
		metvT.par = sfData[dayCurr]->PAR;

		// add calculate Temperature 
		//double tT = simulationPar1(metvT.tday); // ???

		// add calculate ppfd 
		//double ppfdT = simulationPar(psnT.ppfd, i / 24.0 * 60.0 / timeRes);
		psnT.ppfd = calppfdT(cs, epc, &metvT, epv, albedo, sunorshade);
		if (psnT.ppfd) continue;
		psnT.t = sfData[dayCurr]->TA;
		//photosynthesisCoreTimeRes(&psnT, &metvT, &totalA, &totalNum);

		if (photosynthesis(&psnT, metv))
		{
			totalA = totalA + psnT.A;
			++totalNum;
		}		
	}
...
int photosynthesisCoreTimeRes(psn_struct *psn, const metvar_struct* metv, double* totalA, int *totalNum)
{
	/*
	The following variables are assumed to be defined in the psn struct
	at the time of the function call:
...
{
	/*printf("epv->assim_sun: %f,  epvT->assim_sun: %f \n", epv->assim_sun, epvT->assim_sun);
	printf("cf->psnshade_to_cpool: %f,  cfT->psnshade_to_cpool: %f \n", cf->psnshade_to_cpool, cfT->psnshade_to_cpool);
	printf("psn_sun->A: %f,  psn_sunT->A: %f \n", psn_sun->A, psn_sunT->A);
	printf("psn_shade->A: %f,  psn_shadeT->A: %f \n", psn_shade->A, psn_shadeT->A);*/
...
int readStationFluxData(std::vector<StationDataFlux*> &sfData, const char* fluxStationDataFile, const int yearS)
{
	...
	if (infile.is_open()) {
		...
		while (getline(infile, lineStr)) {
			...
			ff->SWC = std::stold(resultStr[10]);
			ff->PAR = std::stold(resultStr[11]);
