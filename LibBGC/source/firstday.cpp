...
epvar_struct* epv, phenarray_struct* phen, cstate_struct* cs, nstate_struct* ns)
{
	...
	double max_stemc,new_stemc,new_rootc;
	double prop_transfer,transfer;
	double prop_litfall;
	//20130907
	double max_crootc,new_crootc;
	...
	/*2014-11-07 如果是草地等非乔木型类型，初始化的根的生物量作为细根生物量。
	if(!epc->woody)
	{
		max_frootc = cinit->croot1988;
	}*/
	//cs->frootc_transfer = cinit->max_leafc * epc->alloc_frootc_leafc * 
	//	epc->froot_turnover;
	cs->frootc_transfer = max_frootc * epc->froot_turnover;
	...
	{		
		...
		max_crootc = cinit->croot1988;		
		new_rootc = cs->leafc_transfer * epc->alloc_crootc_leafc;
		cs->livecrootc_transfer = new_rootc * epc->alloc_newlivewoodc_newwoodc;
		cs->livecrootc = cs->livecrootc_transfer  / epc->livewood_turnover;
		cs->deadcrootc_transfer = new_rootc -cs->livecrootc_transfer;
		cs->deadcrootc = max_crootc - cs->livecrootc_transfer - cs->livecrootc - cs->deadcrootc_transfer;
		
		if(cs->deadcrootc < 0.0)
			cs->deadcrootc = 0.0;
	}
