...
based on literure (Jolly et al, 2005) and own method. 
*/
...

{
	...
	double base_temp = 0;	// epc->base_temp  usually we set it as zero
	...
	for (ny = 0; ny < nyears; ny++)
	{
		...
		for (yday = 0; yday < NDAYS_OF_YEAR; yday++)
		{
			...
			double sn_abs = 0.6;	/* absorptivity of snow */
			double tcoef = 0.65;   /* (kg/m2/deg C/d) temp. snowmelt coef */
			double lh_fus = 335.0;  /* (kJ/kg) latent heat of fusion */
			double lh_sub = 2845.0; /* (kJ/kg) latent heat of sublimation */
