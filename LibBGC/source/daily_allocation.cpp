...
ntemp_struct* nt, double naddfrac, int mode) /* mode is MODE_SPINUP or MODE_MODEL */
{
	...
	/*add 2014-11-20 重新计算分配比例，粗根的部分重新独立分配，不依附于树干*/
	if(woody && c3_flag &&  epc->alloc_newstemc_newleafc==0)
	{
		c_allometry = ((1.0+g1)*(1.0 + f1 + f2 +f3));
		n_allometry = (1.0/cnl + f1/cnfr + (f4*(f3+f2))/cnlw + 
			((1.0-f4)*(f3+f2))/cndw);
	}
	...
	if (woody && c3_flag && epc->alloc_newstemc_newleafc==0)
	{
		...
		cf->cpool_to_deadstemc          = nlc * f3 * (1.0-f4) * pnow;
		cf->cpool_to_deadstemc_storage  = nlc * f3 * (1.0-f4) * (1.0-pnow);
		cf->cpool_to_livecrootc         = nlc * f2 * f4 * pnow;
		cf->cpool_to_livecrootc_storage = nlc * f2 * f4 * (1.0-pnow);
		cf->cpool_to_deadcrootc         = nlc * f2 * (1.0-f4) * pnow;
		cf->cpool_to_deadcrootc_storage = nlc * f2 * (1.0-f4) * (1.0-pnow);
	}
	...
	if (woody && c3_flag && epc->alloc_newstemc_newleafc==0)
	{
		...
		nf->npool_to_deadstemn          = (nlc * f3 * (1.0-f4) / cndw) * pnow;
		nf->npool_to_deadstemn_storage  = (nlc * f3 * (1.0-f4) / cndw) * (1.0-pnow);
		nf->npool_to_livecrootn         = (nlc * f2 * f4 / cnlw) * pnow;
		nf->npool_to_livecrootn_storage = (nlc * f2 * f4 / cnlw) * (1.0-pnow);
		nf->npool_to_deadcrootn         = (nlc * f2 * (1.0-f4) / cndw) * pnow;
		nf->npool_to_deadcrootn_storage = (nlc * f2 * (1.0-f4) / cndw) * (1.0-pnow);
