...
int csummary(cflux_struct* cf, cstate_struct* cs, summary_struct* summary)
{
	...
	summary->daily_root_mr = cf->froot_mr + cf->livecroot_mr;
	summary->daily_leaf_mr = cf->leaf_day_mr +cf->leaf_night_mr;
	...
	/*new added variables leafc,stemc, and crootc yearly. 2013-03-27*/
	summary->leafc_sum = cs->leafc + cs->leafc_storage + cs->leafc_transfer;

	summary->stemc_sum = cs->livestemc + cs->livestemc_storage + cs->livestemc_transfer +
		cs->deadstemc + cs->deadstemc_storage + cs->deadstemc_transfer;
	summary->crootc_sum = cs->livecrootc + cs->livecrootc_storage + cs->livecrootc_transfer +
		cs->deadcrootc + cs->deadcrootc_storage + cs->deadcrootc_transfer;
