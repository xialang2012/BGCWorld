...
nstate_struct* ns, nflux_struct* nf, ntemp_struct* nt)
{
	...
	*/
	...
	//printf("BGCw_scalar=%lf\n",w_scalar);
	//假如是湿地类型，则调整水分的系数。2014-12-08
	if(sitec->epctype ==13)
	{
		//add
		...
		if(soilw_vwc >0.8)
			w_scalar = 0.9 - 0.5 * soilw_vwc;  //y= 0.5 * x+0.9
		if((soilw_vwc > 0.1) && (soilw_vwc <=0.6))
			w_scalar = 1.98 * soilw_vwc - 0.188; //y=1.98 * x-0.188
		if(soilw_vwc <= 0.1) 
			w_scalar = 0.01;	
		if((soilw_vwc > 0.6) && (soilw_vwc <=0.8))
			w_scalar = 2.5 - 2.5 * soilw_vwc; // y = -2.5 * x +2.5
		//printf("土壤水含量=%lf,饱和水含量=%lf，w_scalar=%lf\n",soilw_vwc,sitec->vwc_sat,w_scalar);
	}
	//农田 
	if(epc->epctype == 141)
	{
		//add
		soilw_vwc = (ws->soilw/(1000*sitec->soil_depth))/sitec->vwc_fc;
	...
	/*不同的树种利用不同的分解速率2013-08-24*/
	kl2_base = KL2_BASE * epc->decom_KL2;	/* cellulose litter pool */
	kl4_base = KL4_BASE * epc->decom_KL4;	/* lignin litter pool */

	ks3_base = KS3_BASE * epc->decom_KS3;	/* slow microbial recycling pool */
	ks4_base = KS4_BASE * epc->decom_KS4;	/* recalcitrant SOM (humus) pool */
	kfrag_base = KFRAG_BASE * epc->decom_cwdc; /* physical fragmentation of coarse woody debris */
